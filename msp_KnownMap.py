...
class LidarTest:
    ...
    def run_a_star(self, start_grid_x, start_grid_y, end_grid_x, end_grid_y, rerun = 0):
        ...
        finder = MinimumSpanningTree(diagonal_movement = DiagonalMovement.only_when_no_obstacle)
    ...
    def set_movement(self):
        ...
        for i in range(len(path)):
            ...
            next_grid_x = path[i].y
            next_grid_y = path[i].x
            ...
            self.client.moveToPositionAsync(drone_x, drone_y, drone_z, 3, timeout_sec=10).join()
    ...
    def log_to_file(self, data, path="MSP_Metrics_KnownMap.csv"):
        if(not os.path.exists("MSP_Metrics_KnownMap.csv")):
            header = ['Total Meters', 'Computation Time','Total Runtime', 'Reached Target']
            with open("MSP_Metrics_KnownMap.csv", 'w') as f:
                f.write(f'{header}\n')
        with open("MSP_Metrics_KnownMap.csv", "a") as f:
            f.write(f'{data}\n')                    
